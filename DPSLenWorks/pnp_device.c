...
#define LENWORKS_INTERFACE_A_INDEX 0

#define DEVICEINFO_INDEX 1

#define DEFAULT_SEND_TELEMETRY_INTERVAL_MS 10000

#define DEVICE_CAPABILITY_MODEL_URI "urn:lenworks:azure_pnp_dcm:1"
...
int pnp_device_initialize(const char* connectionString, const char* trustedCert)
{
    ...
    if ((interfaceClientHandles[LENWORKS_INTERFACE_A_INDEX] = Lenworks_interface_aInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
        LogError("Lenworks_interface_aInterface_Create failed");
        return -1;
    }
    ...
    Lenworks_interface_aInterface_Property_ReportAll();
...
void pnp_device_run()
{
    ...
    if (nowTick - lastTickSend >= DEFAULT_SEND_TELEMETRY_INTERVAL_MS)
    {
        ...
        Lenworks_interface_aInterface_Telemetry_SendAll();
...
{
    if (interfaceClientHandles[LENWORKS_INTERFACE_A_INDEX] != NULL)
    {
        Lenworks_interface_aInterface_Close(interfaceClientHandles[LENWORKS_INTERFACE_A_INDEX]);
    }
